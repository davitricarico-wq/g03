...
O R02 trata do vazamento de dados sensíveis (LGPD), focado no acesso indevido a dados de pessoas vulneráveis. Com probabilidade de 11% a 30% e impacto muito alto, o nível de risco é alto. A mitigação envolve o uso de dados fictícios ou anonimizados, boas práticas de armazenamento e cuidado para que informações sensíveis não sejam expostas em repositórios, apresentações ou ambientes públicos. A resposta prevê a remoção imediata dos dados expostos, revisão dos materiais compartilhados e comunicação aos responsáveis pelo projeto.
...
O R04 detalha o erro na precisão da geolocalização de vulneráveis, indicando locais incorretos para resgate. Tem probabilidade de 31% a 50% e impacto muito alto, resultando em risco alto. A mitigação ocorre por meio da captura das coordenadas pelo dispositivo, validação visual das informações registradas e possibilidade de nova captura caso a localização esteja incorreta. A resposta envolve revisar o registro e corrigir manualmente as coordenadas quando necessário.

O R05 refere-se à indisponibilidade do sistema durante eventos climáticos, como queda de servidor ou lentidão. Com probabilidade de 11% a 30% e impacto muito alto, o nível de risco é alto. A mitigação utiliza salvamento local temporário, organização dos dados para uso offline quando possível e testes de estabilidade do sistema. A resposta envolve registrar os dados localmente e sincronizar ou atualizar as informações assim que o sistema estiver disponível novamente.
...
A oportunidade R10 foca na replicação para outros municípios do ABC para expandir a solução para cidades vizinhas. Com probabilidade de 71% a 90% e impacto alto, o aproveitamento será feito estruturando o sistema como plataforma modular para facilitar a parametrização. O R11 prevê a possibilidade futura de compartilhamento de informações com órgãos de apoio, como Corpo de Bombeiros e Polícia Militar, especialmente em situações emergenciais. Possui probabilidade de 51% a 70% e impacto alto, podendo ser aproveitado por meio de relatórios exportáveis ou formatos padronizados de dados, sem depender inicialmente de integração automática por APIs.
