...
| Critério de aceite 1 | CR1: Dado que o gestor acessa o módulo de consultas, quando pesquisa por nome, CPF ou endereço, então a tela deve mostrar os dados básicos sobre o responsável, moradia, moradores e pets relacionados à(s) família(s) encontradas. |
| Critério de aceite 2 | CR2: Dado que o gestor visualiza os dados de uma família, quando a família estiver sem moradia ou sem responsável vinculado, então o sistema deve exibir a flag visual de "cadastro incompleto" no cabeçalho da consulta. |
| Critério de aceite 3 | CR3: Dado que o gestor visualiza a lista de resultados, quando aplica filtros (ex.: núcleo da cidade e/ou bairro, atributos específicos de cidadãos e de moradias, pets), então o sistema deve refinar a lista exibindo apenas os registros que atendem aos filtros selecionados. |
| Critério de aceite 4 | CR4: Dado que o gestor realiza uma busca ou aplica filtros, quando nenhum registro corresponde aos critérios, então o sistema deve exibir a mensagem de "nenhum resultado encontrado" em vez de uma lista vazia. |
| Critérios INVEST | Independente: Utiliza joins de dados já populados pelas US01 e US02.; Negociável: O layout de exibição e os níveis de alerta crítico podem ser ajustados.; Valiosa: Facilita o trabalho do gestor que não precisará cruzar tabelas manualmente em planilhas.; Estimável: A consulta a dados relacionados tem esforço facilmente mensurável.; Pequena: Trata-se de uma view (Visualização de dados) de leitura integrada.; Testável: O teste garante que os dados exibidos batam com o cadastro e que a flag de "cadastro incompleto" apareça quando a família está sem moradia ou sem responsável. |
...
| Critério de aceite 1 | CR1: Dado que o agente entra na tela de cadastro de nova moradia, quando a tela carregar, então o sistema deve capturar automaticamente a posição atual do dispositivo (latitude e longitude) e exibir um mini-mapa de confirmação com a posição capturada e um pin reposicionável que representa o local exato da casa. |
| Critério de aceite 2 | CR2: Dado que o agente esteja cadastrando a moradia a partir de um ponto diferente da residência real, quando interagir com o mini-mapa, então o sistema deve permitir posicionar o pin sobre o local correto da casa — tocando no ponto desejado do mapa ou arrastando o pin —, com retorno visual de que a localização já foi definida, um botão para apagar/reposicionar o pin e outro para re-registrar a posição atual do dispositivo; o ajuste é feito pelo pin no mapa, sem digitação manual de coordenadas. |
| Critério de aceite 3 | CR3: Dado que ainda não há um pin válido definido para a moradia, quando o mini-mapa carregar, então ele deve iniciar centralizado em Santo André e exigir que o agente posicione e confirme o pin antes de concluir o cadastro (coordenadas obrigatórias). |
| Critérios INVEST | Independente: A captura de geolocalização e o mini-mapa de confirmação são independentes dos demais campos de texto do formulário de moradia.; Negociável: A precisão exigida (ex.: margem de 5 a 10 metros) e o mapa-base utilizado podem ser acordados com o time.; Valiosa: Garante coordenadas confiáveis ao permitir confirmar e ajustar visualmente o pin no mapa, eliminando o erro humano na localização da casa.; Estimável: Reutiliza a biblioteca de mapas já prevista (US03) e as APIs nativas de geolocalização, com esforço previsível.; Pequena: Cobre a captura automática da posição e a confirmação/reposicionamento do pin no mini-mapa.; Testável: É possível simular coordenadas em emulador e validar a exibição do mini-mapa, o reposicionamento do pin e o bloqueio do salvamento sem pin confirmado. |
...
| Critério de aceite 1 | CR1: Dado que uma moradia esteja ativa no sistema, quando não houver um morador definido como responsável, então o sistema deve exibir um alerta de inconsistência cadastral e impedir a finalização ou permanência do cadastro como ativo. |
| Critério de aceite 2 | CR2: Dado que um responsável seja arquivado, inativado ou removido da moradia, quando a alteração for confirmada, então o sistema deve exigir a definição de um novo responsável antes de concluir a operação. |
| Critérios INVEST | Independente: A validação do responsável pode ser implementada sem impactar os demais módulos do sistema.; Negociável: As regras de definição e substituição do responsável podem ser ajustadas conforme as políticas da Defesa Civil.; Valiosa: Garante consistência nos registros familiares e melhora a rastreabilidade das informações sociais.; Estimável: A implementação exige apenas validações de vínculo e regras de negócio simples.; Pequena: O escopo está restrito à obrigatoriedade e substituição do responsável.; Testável: O QA pode tentar manter uma moradia ativa sem responsável e validar se o sistema bloqueia a operação corretamente. |
